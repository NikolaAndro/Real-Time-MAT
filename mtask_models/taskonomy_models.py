...
class ResNet(nn.Module):
	def __init__(self, block, layers, tasks=None, num_classes=None, ozan=False, size=1, base_match=512, **kwargs):
		...
		if tasks is not None:
			...
			base = {1 : 512, 2 : 720, 3 : 880, 0.5 : 360}
			rescaled = {k : b * (base_match//512) for k, b in base.items()}

			self.task_to_params = {
				'autoencoder'       	: {'output_channels' : 3,  'downscale' : 1},
				'class_object'      	: {'output_channels' : 0, 'num_classes':1000},
				'class_places'       	: {'output_channels' : 0, 'num_classes':63},
				'depth_euclidean'   	: {'output_channels' : 1,  'downscale' : 1},
				'depth_zbuffer'     	: {'output_channels' : 1,  'downscale' : 1},
				'edge_occlusion'    	: {'output_channels' : 1,  'downscale' : 1},
				...
				'principal_curvature' 	: {'output_channels' : 2,  'downscale' : 1},
				'reshading'         	: {'output_channels' : 1,  'downscale' : 1},
				'room_layout'       	: {'output_channels' : 0, 'num_classes':9},
				'segment_unsup25d'  	: {'output_channels' : 64, 'downscale' : 1},
				'segment_unsup2d'   	: {'output_channels' : 64, 'downscale' : 1},
				'segmentsemantic'   	: {'output_channels' : 18, 'downscale' : 1},
				'vanishing_point'   	: {'output_channels' : 0, 'num_classes':9}
			}

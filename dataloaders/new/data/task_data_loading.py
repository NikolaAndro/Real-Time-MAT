...
from   dataloaders.new.data.load_ops import *
import numpy as np
    ...
    if 'is_discriminative' in cfg or 'only_target_discriminative' in cfg and cfg['only_target_discriminative']:
        ...
        if 'depth_mask' in cfg and cfg['depth_mask']:
            depth_values = load_raw_image(
                make_filename_for_domain( filename_template, 'depth_euclidean' ),
                color=False,
                use_pil=use_pil)
...
def make_mask( input_img, target_img, cfg, mask_dim=None ):
    ...
    target_mask = resize_image( target_mask, mask_dim, interp_order=cfg['resize_interpolation_order'] ).copy() # Resize along with target

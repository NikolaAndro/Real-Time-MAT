...
class TaskonomyLoader(data.Dataset):
	...
	INFO = {"mean": [0.485, 0.456, 0.406],
			"std": [0.229, 0.224, 0.225]} #TODO: Change the info since used in PGD attack
	def __init__(self,
				 root,
				 is_training,
				 threshold,
		...
		for where, subdirs, files in os.walk(os.path.join(root, 'rgb')): #there is a folder rgb and images are stored there
			# print(where)
	...
	def __getitem__(self, index):
		...
		# import torchvision
		# torchvision.utils.save_image(im,"/home/namdd/Documents/github_NOT_ALL_backed_up/paper/MAT-Attack/MAT_Attack/saved_models/"+"im_1.jpg")
		# im, _, _ = cfg['preprocess_fn'](file_name.replace('rgb', cfg['target_domain_name']), cfg)
		im, _, _ = cfg['preprocess_fn'](file_name, cfg)
		# torchvision.utils.save_image(im,"/home/namdd/Documents/github_NOT_ALL_backed_up/paper/MAT-Attack/MAT_Attack/saved_models/"+"im_1.jpg")
		# print(cfg['target_domain_name'])
		# print(file_name.replace('rgb', cfg['target_domain_name']))
		im = self.process_image(im, flip)
		# torchvision.utils.save_image(im,"/home/namdd/Documents/github_NOT_ALL_backed_up/paper/MAT-Attack/MAT_Attack/saved_models/"+"im_2.jpg")
		ys = {}
		masks = {}
		...
		for task in self.task_set:
			...
			# print("Loading file name: ",file_name.replace('rgb', cfg['target_domain_name']))
			try:
				yim, mask = load_target(file_name.replace('rgb', cfg['target_domain_name']), im, cfg)
			except:
				print("------------Error : -----------", cfg)
